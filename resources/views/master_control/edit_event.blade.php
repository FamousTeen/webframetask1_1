...
<div class="max-w-3xl mx-auto mt-5">
  ...
  <form id="eventForm" action="{{ route('events.update', ['id' => $selectedEvent->id, 'event' => $selectedEvent]) }}"
    method="POST">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div>
        ...
        <input type="date" id="event-date" name="event_date"
          class="mt-1 block w-full p-2 border border-gray-300 rounded-md" value="{{ $selectedEvent->date }}">
      </div>
      ...
      <div>
        ...
        <input type="text" id="location" name="location" class="mt-1 block w-full p-2 border border-gray-300 rounded-md"
          placeholder="Enter location" value="{{ $selectedEvent->venue }}">
      </div>
      ...
      <div>
        ...
        <input type="time" id="event-time" name="event_time"
          class="mt-1 block w-full p-2 border border-gray-300 rounded-md" value="{{ $selectedEvent->start_time }}">
      </div>

      <!-- Organizer -->
      <div>
        <label for="organizer" class="block text-sm font-medium text-gray-700">Organizer</label>
        <select id="organizer" name="organizer" class="mt-1 block w-full p-2 border border-gray-300 rounded-md">
          <option value="{{$selectedEvent->organizer->organizer_id }}">{{ $selectedEvent->organizer->name }}</option>
          @foreach ($organizers as $organizer)
          @if ($organizer->organizer_id == $selectedEvent->organizer->organizer_id)
            <?php continue; ?>
          @else
      ...
      <div>
        ...
        <input type="text" id="booking-url" name="booking_url"
          ...
          value="{{ $selectedEvent->booking_url }}">
      </div>
      ...
      <div class="md:col-span-2">
        ...
        <textarea id="about" name="about" rows="4" class="mt-1 block w-full p-2 border border-gray-300 rounded-md"
          ...
          value="{{ $selectedEvent->description }}">{{ $selectedEvent->description }}</textarea>
      </div>

      <!-- Tags -->
      <div class="md:col-span-2">
        <label for="tags" class="block text-sm font-medium text-gray-700">Tags</label>
        <?php $all_tag = DB::table('tags')->where('event_id', $selectedEvent->event_id)->get(); ?>
        @if (!($all_tag->isEmpty()))
      <div class="bootstrap-tagsinput">
        @foreach ($all_tag as $tag)
      <span class="tag label label-info">{{$tag->title}}<span data-role="remove"></span></span>
    @endforeach
      </div>
    @endif
